...
class PDcontrolller:
    def __init__(self):
        # hover control gains
        self.Kp = np.diag([7, 7, 58])   #[7, 7, 58]
        self.Kd = np.diag([4.3, 4.3, 18])   # [4.3, 4.3, 18]
    ...
    def control(self, flat_output, state):
        ...
        error_vel = state.get('v') - flat_output.get('v')

        error_vel = np.array(error_vel).reshape(3,1)
        error_pos = np.array(error_pos).reshape(3,1)

        # page 29
        rdd_des = np.array(flat_output.get('x_ddot')).reshape(3,1) - self.Kd @ (error_vel) - self.Kp @ (error_pos)
        ...
        # print(np.array(flat_output.get('x_ddot').reshape(3,1)))
        # print(error_vel)
        # print(error_pos)
        ...
        error_angle = np.matmul(self.Kp_t, np.array([phi_des, theta_des, psi_des])-angle) + np.matmul(self.Kd_t, np.array([0, 0, flat_output['yaw_dot']])-omega)
        # print(prit)
