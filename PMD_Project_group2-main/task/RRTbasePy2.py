...
class RRTMap:  # for visualisation. Methods draw the map, obstacles and path
    ...
    def drawObs(self, obstacles):  # draws obstacles
        ...
        while (len(obstaclesList) > 0):
            ...
            #pygame.draw.circle(self.map, self.grey, (600, 200), 30)
            #pygame.draw.circle(self.map, self.grey, (500, 400), 30)
            #pygame.draw.circle(self.map, self.grey, (550, 300), 30)
            #pygame.draw.circle(self.map, self.grey, (500, 200), 30)
            #pygame.draw.circle(self.map, self.grey, (600, 400), 30)
            #pygame.draw.polygon(self.map, self.grey, ((300, 200), (300, 500), (400, 200)))
            #pygame.draw.polygon(self.map, self.grey, ((700, 400), (800, 400), (800, 100)))
...
class RRTGraph:  # this class contains the methods that provide the RRT functionality
    ...
    def crossObstacle(self, x1, x2, y1,
                      y2):  # Checks if an edge crosses an obstacle. Returns False when it collides, Free when it's free.
        obs = self.obstacles.copy()
        while (len(obs) > 0):
            rectang = obs.pop(0)
    ...
    def step(self, nnear, n,
             dmax=35):  # move node closer to tree when it's too far and check if goal has been reached (dmax = max distance from any node in the tree, default 35)
    ...
    def path_to_goal(
            self):  # If goal has not been reached, return False. If goal has been reached: creates a list containing the path (from goal to start)
        if self.goalFlag:
            self.path = []
    ...
    def bias(self,
             ngoal):  # Does a step straight towards the goal. This speeds up the algorithm when it is not used too much: now every 10th step (can be changed in RRT.py).
